...
${productDescription ? `PRODUCT DESCRIPTION: ${productDescription}` : ''}`;
  ...
  // GeminiGen clips are always 10 seconds. targetDuration is locked to 10.
  const safeDuration = Math.max(10, Math.round((targetDuration || 10) / 10) * 10);
  // For 10s video: 3 scenes (~3s hook + ~4s body + ~3s CTA). Rule: ~3.5s per scene, min 3, max 15.
  const targetSceneCount = Math.min(15, Math.max(3, Math.round(safeDuration / 3.5)));
...
CRITICAL OUTPUT CONSTRAINT: The final output is ONE 10-second continuous video clip generated by GeminiGen grok-3.
- Total video = exactly 10 seconds
- All scenes must fit within this 10-second window
- imagePrompts are KEYFRAME REFERENCES for GPT-image-2 — each represents a visual moment within the 10s clip
- Do NOT write scenes as if each is a standalone video; they are moments inside one 10s continuous clip
...
TARGET OUTPUT: ${safeDuration} seconds total, ${targetSceneCount} scenes (e.g. 0-3s hook · 3-7s main · 7-10s CTA — distribute evenly, durations must add up to exactly ${safeDuration}s)
...
4. ADAPTED SCENES — exactly ${targetSceneCount} scenes, durations must sum to ${safeDuration}s:
   - duration: e.g. "0-3s", "3-7s", "7-10s" — must add up to exactly ${safeDuration}s total
   - voiceover: Bahasa Indonesia VO text for this moment. ALWAYS Indonesian.
   - imagePrompt: English keyframe reference for GPT-image-2 — describes the VISUAL STATE of this moment in the 10s clip. Rules:
     ${assetMode === 'character'
